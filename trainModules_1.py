...
r_curve = l_des/3
params_init = (
    (np.abs(x_param)<=r_curve)*(np.abs(y_param)<=r_curve)*
    ((x_param-r_curve)**2+(y_param+r_curve)**2 >= (r_curve*2-w_wg)**2/4) * 
    ((x_param-r_curve)**2+(y_param+r_curve)**2 <= (r_curve*2+w_wg)**2/4) +
    (np.abs(x_param)<=w_wg/2)*(y_param<=-r_curve) + 
    (np.abs(y_param)<=w_wg/2)*(x_param>=r_curve)+ 0.0
)
...
# params_init = (
#     (np.abs(x_param)<w_wg/2) + 
#     (np.abs(y_param)<w_wg/2)  * (x_param>0) + 
#     (np.abs(x_param)<w_junction/2) * (np.abs(y_param)<w_junction/2) * (x_param>0) * 
#     ((np.abs(x_param)-w_junction/2)**2 + (np.abs(y_param)-w_junction/2)**2 >= ((w_junction-w_wg)**2/4)) + 0.0
# )
## >2 ver2
# params_init = (
...
params_init += (np.random.uniform(low=-0.5, high=0.5, size=(nx,ny)))
...
for ii in range(history["iterations"], num_steps+15):
    print("---------------------------------")
...
sim_data_opt = td.web.api.webapi.load(
    task_id="fdve-fbd15d57-4206-42f5-aaa2-6db38d082e5d", # "1" at 149
)
